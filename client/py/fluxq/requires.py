...
fabric (as a requires.network subsystem, matched per-subsystem by fluxq).
...
# match your fluxq registration (datagen: efa / infiniband / ethernet).
...
def network_section(options: list[str]) -> list | None:
    ...
    anyof(...) when several are acceptable (let fluxq pick), else a single type."""
